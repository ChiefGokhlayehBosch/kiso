 ...
 * serial interface.
 */
#define ENGINE_SEND_TIMEOUT (pdMS_TO_TICKS(1000))
...
static bool IsEchoModeEnabled = false; //!< State of AT echo mode (on/off)
...
static Retcode_T WriteOntoSerial(const void *data, size_t length, size_t *numBytesWritten)
{
    ...
    BaseType_t result = xSemaphoreTake(TxWakeupSignal, ENGINE_SEND_TIMEOUT);
    if (pdPASS != result)
    {
...
static void ListenForUrcDuringIdle(void)
{
     ...
     * not, or when the command-sender releases the transceiver, we take
     * ownership and interpret whatever is in there as URC. */
...
Retcode_T Engine_Initialize(void)
{
    ...
    if (RETCODE_OK != status)
    {
        LOG_FATAL("Failed to initialize AT transceiver!"); //LCOV_EXCL_BR_LINE
        return status;
    }
...
Retcode_T Engine_OpenTransceiver(struct AtTransceiver_S **atTransceiver)
{
    ...
    enum AtTransceiver_WriteOption_E options;
    ...
    options = (enum AtTransceiver_WriteOption_E)(ATTRANSCEIVER_WRITEOPTION_DEFAULT | ATTRANSCEIVER_WRITEOPTION_NOBUFFER);
    if (!IsEchoModeEnabled)
    {
        options = (enum AtTransceiver_WriteOption_E)(options | ATTRANSCEIVER_WRITEOPTION_NOECHO);
    }
...
void Engine_Deinitialize(void)
{
    ...
    AtTransceiver_Deinitialize(&Transceiver);
}
